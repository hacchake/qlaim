...
for (const md of (globalThis.FUZZ_MODES || ['PLANE','CUBE','SPHERE','GSD','KLEIN','MOBIUS','KNOT','TORUS','TRUNC_ICOSA'])) {
  settings.mode = md;
  for (let game = 0; game < 2; game++) {
    ...
    for (let f = 0; f < 2500; f++) {
      frames++;
      if (f % 20 === 0) { if (d) releaseDir(d); d = DIRS[(Math.random()*4)|0]; pressDir(d); held.fast = Math.random() < 0.4; }
      try {
      ...
      if (state === 'play' && claimed !== initOpen - countOpen() - trail.length) {
        problems.push(md + ' 整合 claimed=' + claimed + ' vs ' + (initOpen - countOpen() - trail.length) + ' lv' + level); break;
      }
